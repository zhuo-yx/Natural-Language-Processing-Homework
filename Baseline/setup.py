...
def pre_process(args):
    ...
    if args.include_test_examples:
        ...
        save(args.test_eval_file, test_eval, message="test eval")  # 测试集处理结果
        test_meta = build_features(args, test_examples, "test",
                                   args.test_record_file, word2idx_dict, char2idx_dict, is_test=True)
        save(args.test_meta_file, test_meta, message="test meta")   # 测试集的meta信息文件

    save(args.word_emb_file, word_emb_mat, message="word embedding")  # word embedding矩阵  (word_voc_size, embedding_size)
    save(args.char_emb_file, char_emb_mat, message="char embedding")  # char embedding矩阵 (char_voc_size, embedding_size)
    save(args.train_eval_file, train_eval, message="train eval")    # 训练集处理结果
    save(args.dev_eval_file, dev_eval, message="dev eval")  # 验证集处理结果
    save(args.word2idx_file, word2idx_dict, message="word dictionary")  # word2index文件
    save(args.char2idx_file, char2idx_dict, message="char dictionary")  # char2index文件
